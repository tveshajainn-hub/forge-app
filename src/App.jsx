...
function YTThumb({yt,bp,height=100}){
  const [err,setErr]=useState(false);const color=BP[bp]?.color||C.accent;
  const open=()=>{if(yt)window.open(`https://www.youtube.com/watch?v=${yt}`,"_blank");};
  if(!yt||err)return<div style={{height,borderRadius:12,background:`${color}15`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:36,border:`1px solid ${color}25`,cursor:"pointer"}} onClick={open}>{BP[bp]?.icon||"💪"}</div>;
  return(<div onClick={open} style={{position:"relative",height,borderRadius:12,overflow:"hidden",cursor:"pointer"}}><img src={`https://img.youtube.com/vi/${yt}/mqdefault.jpg`} onError={()=>setErr(true)} alt="" style={{width:"100%",height:"100%",objectFit:"cover",display:"block"}}/><div style={{position:"absolute",inset:0,background:"linear-gradient(to top,rgba(6,8,15,.8),transparent 50%)",display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{width:40,height:40,background:"#FF0000CC",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{color:"#fff",fontSize:14,marginLeft:2}}>▶</span></div></div></div>);
}
          ...
          return(<div key={i} style={{background:done?C.card2+"AA":C.card,border:`1.5px solid ${done?C.border:isExp?dayPlan.color:C.border}`,borderRadius:16,marginBottom:8,overflow:"hidden",transition:"all .25s"}}>
            <div style={{display:"flex",alignItems:"flex-start",gap:12,padding:"12px 14px",cursor:"pointer"}} onClick={()=>setExpanded(e=>({...e,[i]:!isExp}))}>
              ...
              <div style={{display:"flex",gap:6,alignItems:"center",flexShrink:0}}>
                {info.yt&&<button onClick={ev=>{ev.stopPropagation();window.open(`https://www.youtube.com/watch?v=${info.yt}`,"_blank");}} style={{width:26,height:26,background:"#FF0000CC",borderRadius:7,border:"none",cursor:"pointer",color:"#fff",fontSize:9,display:"flex",alignItems:"center",justifyContent:"center"}}>▶</button>}
            ...
            {isExp&&(<div style={{padding:"0 14px 14px",borderTop:`1px solid ${C.border}`,background:"rgba(0,0,0,.25)"}}>
              {info.yt&&<div style={{marginBottom:10}}><YTThumb yt={info.yt} bp={ex.bp||info.bp} height={100}/></div>}
    ...
    <div style={{padding:"0 20px"}}>
          ...
          {Object.entries(EX).filter(([,i])=>i.bp===selBP).map(([name,info])=>{const sessions=(data.strength||[]).filter(e=>e.exercise===name).sort((a,b)=>b.date<a.date?-1:1);const best=sessions.reduce((m,e)=>Math.max(m,parseFloat(e.weight)||0),0);const isSel=selEx===name;const target=(data.targets||{})[name];const tPct=target&&best>0?Math.min(100,Math.round(best/parseFloat(target)*100)):null;
            return(<div key={name} style={{background:isSel?C.card2:C.card,border:`1.5px solid ${isSel?BP[selBP]?.color:C.border}`,borderRadius:14,marginBottom:8,overflow:"hidden"}}>
              <div onClick={()=>setSelEx(isSel?null:name)} style={{padding:"12px 14px",cursor:"pointer",display:"flex",gap:12,alignItems:"center"}}>
                ...
                {info.yt&&<button onClick={ev=>{ev.stopPropagation();window.open(`https://www.youtube.com/watch?v=${info.yt}`,"_blank");}} style={{width:26,height:26,background:"#FF0000",borderRadius:7,border:"none",cursor:"pointer",color:"#fff",fontSize:9,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>▶</button>}
              </div>
              {isSel&&(<div style={{padding:"0 14px 14px",borderTop:`1px solid ${C.border}`}}>
                {info.yt&&<div style={{marginBottom:10}}><YTThumb yt={info.yt} bp={selBP} height={90}/></div>}
      ...
      {tab==="log"&&<>
        ...
        {showForm&&<Card style={{marginBottom:14}}>
          ...
          {form.exercise&&EX[form.exercise]?.yt&&<div style={{marginBottom:10}}><YTThumb yt={EX[form.exercise].yt} bp={EX[form.exercise].bp} height={85}/></div>}
